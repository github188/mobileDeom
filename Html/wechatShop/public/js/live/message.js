...
var wsUri = "ws://ws.t5166.com:9502";
...
function danmu(data){
    ...
    dm_t = setTimeout(function(){
        ...
        setTimeout(function(){$('.discuss li').remove()},500);
    },15000)
}

function fahongbao(data){
    queue_id = data.queue_id;
    $('.hongbao p').html("红包在"+queue_id+"号商品中");
    $('.hongbao').css({"left":0});
    clearInterval(default_time);
    setTimeout(function(){$('.hongbao').css({"left":-400})},25000);
}

function hongbaoyugao(data){
    $('.hongbao p').html("红包将在<span>59</span>秒后到达");
    $('.hongbao').css({"left":0});
    clearInterval(default_time);
    timeout = setInterval(function(){
        time = parseInt($('.hongbao p span').html())-1;
        $('.hongbao p span').html(time);
        if(time==0){
            $('.hongbao').css({"left":-400});
