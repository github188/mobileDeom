...
//      获取收藏列表
        $question = \Mobile\Controller\StudyController::getNewQuestion($where);


        $this->assign('allquestion',$question);
    ...
    public function faqCourse(){
        ...
        $dataList = $video_shoucang_db->join('ht_video v on v.id = s.vid')->field('s.id, v.id as vid,v.title,s.addtime')->where(array('s.user_id' => $user_id))->select();
