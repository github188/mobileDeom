...
+function () {
    function mySel(ll,is) {
        ...
        this.fn = function () {
            $(ll).on("click",function (e) {
                ...
                if (is ==1) {
                    var num = $e.attr('data-num'); //获取金额
                    $('#recMoney').val(num);
                }else if(is ==2){
                    var pay_url = $e.find('a').attr('data-pay-url'); //获取支付URL
                    $e.parents('form').attr('action',pay_url);
                }
            });
        }
    }
    ...
    mSelA.fn();
    //myRecharge01.html
    var mSelB = new mySel('.selList ul li', 1);
    mSelB.fn();
    //myRecharge02.html
    var mSelC = new mySel('.recStyle ul li', 2);
    mSelC.fn();
}();
