...
class ImageCut  {  
	 ...
	 * 构造器  
	 */  
	public function __construct($filename,$x,$y,$x1,$y1,$w,$h){  
		$this->initialize($filename,$x,$y,$x1,$y1,$w,$h);
		//log_message('debug', "Img_shot Class Initialized");  
	} 

	/** 
	 * 初始化截图对象 
	 *@param filename 源文件路径全名 
	 *@param width  截图的宽 
	 *@param height  截图的高 
	 *@param x  横坐标1 
	 *@param y  纵坐标1 
	 ...
	 *  
	 */  
	public function initialize($filename,$x,$y,$x1,$y1,$w,$h){  
		if(file_exists($filename)){  
			$this->filename = $filename;  
			$pathinfo = pathinfo($filename);  
			$this->ext = $pathinfo['extension'];  
		}else{  
			$e = new Exception('文件不存在!'.$filename,1050);  
			throw $e;  
		}  
		$this->x = $x;  
		$this->y = $y;     
	...
	private function get_shot_name() {  
		...
		$filename = $pathinfo['filename'] . '-small.' . $pathinfo['extension'];  
	...
	private function generate_png() {  
		...
		if(imagecopyresampled($dst_r,$img_r,0,0,$this->x,$this->y,$this->width,$this->height,$this->width,$this->height)){
