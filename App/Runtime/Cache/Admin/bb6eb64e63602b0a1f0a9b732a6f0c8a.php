            ...
            <div class="tab-content">
                <div id="tab_1" class="tab-pane active">
                    <div class="pull-right" style="display:block;margin:15px 0px 10px 0px;">
                        <a class="btn btn-info" href="<?php echo U(MODULE_NAME.'/User/group_add');?>" >
                        ...
                        <tbody>
                            <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="altbg2" onmouseover="this.className='altbg1'" onmouseout="this.className='altbg2'">
                                <td style="border-bottom:1px dotted #EDEDED">
                                    <input name="ugid[]" class="checkbox" type="checkbox" value="<?php echo ($v['ugid']); ?>">
                                    <?php echo ($v['ugid']); ?>
                                </td>
                                <td style="border-bottom:1px dotted #EDEDED"><?php echo ($v['name']); ?></td>
                                <td style="border-bottom:1px dotted #EDEDED"><?php echo ($v['gqtime']); ?></td>
                                <td style="border-bottom:1px dotted #EDEDED"><?php echo ($v['tg_money']); ?></td>
                                <td style="border-bottom:1px dotted #EDEDED"><?php echo ($v['tg_money_2']); ?></td>
                                <td style="border-bottom:1px dotted #EDEDED"><?php echo ($v['allow_buy'] == 1?是:否); ?></td>
                                <td style="border-bottom:1px dotted #EDEDED"><?php echo ($v['buy_price']); ?></td>
                                <td style="border-bottom:1px dotted #EDEDED">
                                  <a href="<?php echo U(MODULE_NAME.'/User/lists',array('ugid'=>$v[ugid]));?>">[查看该组会员(<font color="#FF0000"><?php echo ($v['counts']?:0); ?></font>)]</a> &nbsp;
                                  <a href="<?php echo U(MODULE_NAME.'/User/group_edit',array('ugid'=>$v[ugid]));?>">[编辑]</a> &nbsp; 
                                  <a href="<?php echo U(MODULE_NAME.'/User/group_delete',array('ugid'=>$v[ugid]));?>" onclick="return confirm('你确实要删除吗?不可恢复');">[删除]</a> &nbsp; 
                                </td>
                              </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                            <?php if(is_array($info)): ?><tr>
                                <td colspan="8">
                                <input type="button" onclick="checkall(this.form)" value="反选"/>
                                <input type="submit" name="dosubmit" onclick="document.myform.action='<?php echo U(MODULE_NAME.'/User/group_delete');?>';return confirm('确认删除这些视频吗？')" value="批量删除"/>
                                </td>
                                </tr><?php endif; ?>
                            <?php if(!$info): ?><tr>
                            ...
                            <?php if($totalPages > 1): ?><tr>
                                    <td colspan="8" align="center"><div class="pages"><?php echo ($page); ?></div> </td>
                                </tr><?php endif; ?>
                        </tbody>
                    </table>
                    </form>
