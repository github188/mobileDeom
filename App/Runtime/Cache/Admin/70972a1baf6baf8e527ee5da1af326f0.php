...
<div class="wrapper">
			...
			<div class="col-xs-12">
				<div class="box box-info"> <!-- box-info天空蓝、 box-primary蓝色、  box-success绿色-->
					<div class="box-header nav-tabs">
						...
						<?php if($user['username'] != session('uname')): ?><!-- 返回上一页 -->
	...
	<a class="btn btn-default" title="返回上一页" onclick="javascript:history.back(-1);" href="#">
		<i class="fa fa-reply"></i>
					...
					<!-- 表格开始 -->
					<div class="box-body">
						<form action="<?php echo U(MODULE_NAME.'/Rbac/editUser');?>" method="post">  
							<table id="example" class="table table-bordered">
								<tbody>
									...
									<tr>
										...
										<td><input type="text" name="name" value="<?php echo ($user["name"]); ?>" class="form-control" size="30" data-rule="required;length[2~]"></td>
									</tr>
									<tr>
										<td class="table-td-right"> 性别：</td>
										<td>
						                    男 <input type="radio" name="sex" id="iCheck1" value="0" <?php if(!$user['sex']): ?>checked<?php endif; ?>> &nbsp;
						                    女 <input type="radio" name="sex" id="iCheck2" value="1" <?php if($user['sex']): ?>checked<?php endif; ?>>
										</td>
									</tr>
									<tr>
										<td class="table-td-right"> Email邮箱：</td>
										<td><input type="text" name="email" value="<?php echo ($user["email"]); ?>" class="form-control" size="30" data-rule="email"></td>
									</tr>
									<tr>
										<td class="table-td-right"> 手机号码：</td>
										<td><input type="text" name="tel" value="<?php echo ($user["tel"]); ?>" class="form-control" size="30" data-rule="mobile"></td>
									</tr>
									<?php if($super and $user['username'] != session('uname')): ?><tr>
											<td class="table-td-right"> 锁定状态：</td>
											<td>
							                    正常 <input type="radio" name="lock" id="iCheck1" value="0" <?php if(!$user['lock']): ?>checked<?php endif; ?>> &nbsp;
							                    锁定 <input type="radio" name="lock" id="iCheck2" value="1" <?php if($user['lock']): ?>checked<?php endif; ?>>
											</td>
										</tr>
										<tr>
											...
											<td>
							                    <select name="role_id" required>
							                        ...
							                        <?php if(is_array($role)): foreach($role as $key=>$v): ?><option value="<?php echo ($v['id']); ?>" <?php if(is_array($user["role"])): foreach($user["role"] as $key=>$value): if($v["id"] == $value["id"]): ?>selected="selected"<?php endif; endforeach; endif; ?>><?php echo ($v["name"]); ?>&nbsp;[<?php echo ($v["remark"]); ?>]</option><?php endforeach; endif; ?>
							                    </select>
											</td>
										</tr><?php endif; ?>
									<tr>
									...
									<?php if($super and $user['username'] != session('uname')): ?><tr>
										...
										<tr>
											...
											<td><?php echo (date("Y-m-d H:i:s",$user["up_time"])); ?></td>
										</tr><?php endif; ?>
									<tr height="60">
