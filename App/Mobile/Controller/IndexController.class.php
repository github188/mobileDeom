...
class IndexController extends EmptyController {
    ...
    public function index(){
        ...
        $this->coursePrice = $userGroup_db->where(array('allow_buy'=>1))->cache(2)->limit(4)->order('sort ASC,buy_money DESC,id DESC')->select();
        ...
        $this->banner = M('ad')->where(array('is_show'=>1,'area'=>3,'is_alert'=>0))->cache(2)->order('sort DESC,id DESC')->select();//轮播图广告
        $this->videoshow = $video_db->where('status=0')->cache(2)->limit(8)->order('is_tag DESC,id DESC')->select();//热门视频
        $this->student = $article_other_db->where(array('type'=>2,'is_show'=>1))->cache(2)->limit(4)->order('sort ASC,id DESC')->select();//学员介绍
        $this->notice  = $article_other_db->where(array('type'=>3,'is_show'=>1))->cache(2)->limit(7)->order('sort ASC,id DESC')->select();//平台公告
        $this->ganhuo  = $article_other_db->where(array('type'=>4,'is_show'=>1))->cache(2)->limit(7)->order('sort ASC,id DESC')->select();//干货分享
        $this->problem = $article_other_db->where(array('type'=>5,'is_show'=>1))->cache(2)->limit(7)->order('sort ASC,id DESC')->select();//常见问题

        #最新问答
        $newQuestion = StudyController::getNewQuestion('', 2);

        $this->assign('allquestion', $newQuestion);
    ...
    public function course(){
        $article_other_db  = M('article_other');
        $userGroup_db      = M('user_group');
        $userGroup     = $userGroup_db->where(array('allow_buy'=>1))->cache(2)->order('sort ASC,buy_money DESC,id DESC')->select();
        $userGroupInfo = array();
    ...
    public function videoList()
    {
        ...
        $search_tg_category?cookie('search_tg_category',$search_tg_category):cookie('search_tg_category',null);
//
//
...
//        dump($where);   ->order('is_tag DESC,id DESC')

        $video = $video_db->where($where)->limit(11)->select();
//        dump($video);die;
...
//
            ...
            $type            = cookie('type');    #收费类型
            $key_word        = cookie('key_word');#视频标题
            $search_tg_category        = cookie('search_tg_category');#视频标题
//
            ...
            $video = $video_db->where($where)->limit(($pageSize * 11)+1,12)->select();
//        echo $video_db->getLastSql();die;
            foreach($video as  $k=>$v){
                #判断视频是否被收藏
                $video[$k]['shoucang'] = $video_s_db->where(array('user_id'=>$user_id,'vid'=>$v['id']))->getField('id');
                $video[$k]['playAuth'] = $user_id ? videoPlayAuth($v['id']) : '';

            }

            if (!empty($video)){
//                dump($video);
                $this->ajaxReturn(array('status' => 1, 'data' => $video));
            }else{
