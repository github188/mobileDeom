...
//            //邀请注册
    ...
    public function password(){
        ...
        if(IS_POST){
            $code        = trim(I('post.code'));
            $password    = trim(I('post.pwd1'));
            $password2    = trim(I('post.pwd2'));
            $user_db       = M('user');
            if($password != $password2){
                $this->ajaxReturn(array('msg'=>'两次密码不一致','status'=>0));
            }
            ...
            if(($mobile and $mobile['time'] + 5 * 60 < time()) or ($email and $email['time'] + 15 * 60 < time())){
                $this->error('验证超时！请重新发送验证码', $url);
            }
            $this->assign($mobile?'sendMobile':'sendEmail',$mobile?$mobile['mobile']:$email['email']);
            $this->display();
        }
    }
