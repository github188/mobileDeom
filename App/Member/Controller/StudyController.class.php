...
class StudyController extends CommonController {
    ...
    public static $replyPageSize = 5; //回复每页条数
    public static $questionPageSize = 5; //问题每页条数
    public static $notePageSize = 10; //笔记每页条数
...
//            url解码显示
            $question[$k]['question'] = htmlspecialchars_decode(stripslashes($question[$k]['question']));
...
//                    获取用户默认显示的用户组id
            ...
            $answer[$key]['answer'] = htmlspecialchars_decode(stripslashes($answer[$key]['answer']));
...
//      排序，將0排到最後
    ...
    function writeQuestion(){
        if (IS_AJAX){
            ...
            $result = M('video_question')->add(array('user_id' => $user_id, 'vid' => $vid, 'question' => $content, 'addtime' => $time, 'category_id' => $category_id));
            if ($result){
                $this->ajaxReturn(array('status' => 1,'addtime' => date('Y-m-d H:i',$time), 'id' => $result, 'face' => session(C('USER_FACE')), 'username' => session(C('USER_AUTH_UNAME')), 'user_group_id' => session(C('USER_GROUP_ID'))));
            }else{
                $this->error('发表失败');
            }
        }
...
//          当前用户信息
            $userdata = M('user')->where(array('id' => $user_id))->field('id, username, face')->find();

//          当前问答的视频ID
            $question_info = $VideoQuestionDb->where(array('id' => $question_id))->Field('vid, user_id')->find();
            $user_son_id = $question_info['user_id'];
            $data = array('question_id' => $question_id, 'user_id' => $user_id, 'vid' => $question_info['vid'],'answer' => $answer, 'addtime' => $time);
            ...
            if ($answer_id) {
                ...
                if ($user_id == $reply_user['user_id']){
                    $this->ajaxReturn(array('code' => 0, 'msg' => '不能回复自己哦'));
                }
...
//                添加用户佣金记录
                ...
                if (!$add_result){
                    $this->ajaxReturn(array('code' => 0, 'msg' => '增加佣金记录失败'));
                }
...
//        笔记总数
    ...
    function notePage(){
        if (IS_AJAX){
            ...
            if ($noteList['noteList']){
                ...
                $this->ajaxReturn(array('code' => 1, 'data' => $noteList));
            }else{
                $this->ajaxReturn(array('code' => 0, 'msg' => '_____有更多了_____'));
            }
    ...
    function writeNote(){
        if (IS_AJAX){
            ...
            if (!$playtime){
                $this->error('请播放视频后再发表笔记！');
            }

            $result = M('video_note')->add(array('user_id' => $user_id, 'vid' => $vid, 'content' => $content, 'addtime' => $time, 'playtime' => $playtime));
            if ($result){
                $this->ajaxReturn(array('status' => 1,'addtime' => date('Y-m-d H:i',$time), 'id' => $result, 'playtime' => video_time_change($playtime)));
            }else{
                $this->error('发表失败');
            }
        }
